...
void UVAQuery(const int64_t* d_indices,
              const float* d_db,
              const int emb_dim,
...
public:
  ...
  void Query(const ID_Type* d_indices,
             int key_len,
             float* d_value,
             cudaStream_t stream) {
...
public:
  GPUCacheWithNoHashTorch(
      int64_t capacity, int64_t emb_dim, int64_t start, int64_t end) {
  ...
  void Query(torch::Tensor keys, torch::Tensor values) {
    ...
    cache_->Query(keys.data_ptr<int64_t>(),
                  keys.size(0),
