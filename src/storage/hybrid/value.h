...
public:
          ...
          shm_capacity_bytes_(shm_capacity)
    {
        ...
        std::string index_type = index_config.json_config_.value("index_type","DRAM");
    ...
    ~ValueManager() {
        ...
        if (fd_ssd >= 0) { ::fdatasync(fd_ssd); ::close(fd_ssd); }
    }
    ...
    void WriteValue(Key_t key, const std::string_view& value, unsigned tid) {
        ...
        if (!p_write) {
            { std::lock_guard<std::mutex> lk(evict_mu_); need_evict_ = true; }
        ...
        if (!p_write) p_write = WriteDisk(value);
        if (!p_write) { LOG(ERROR) << "Failed to allocate memory for key " << key; return; }
        ...
        if (old_raw) FreePointer(UnifiedPointer::FromRaw(old_raw));
    }
        ...
        switch (p.type()) {
            ...
            case UnifiedPointer::Type::Disk: {
                ...
                if ( (std::hash<Key_t>{}(key) & 0x7) == 0 ) {
                    ...
                    if (hot_.insert(acc, key)) acc->second = 0;
                    if (acc->second < kPromoteThreshold) acc->second++;
                }
...
private:
    ...
    size_t HighWatermark() const { return (size_t)(shm_capacity_bytes_ * kHighWatermarkRatio); }
    size_t LowWatermark()  const { return (size_t)(shm_capacity_bytes_ * kLowWatermarkRatio);  }
    ...
    void FreePointer(const UnifiedPointer& p) {
        if (!p) return;
        if (p.type() == UnifiedPointer::Type::PMem) return; // inline 小值无需释放
        ...
        if (p.type() == UnifiedPointer::Type::Memory) {
            ...
            shm_used_bytes_.fetch_sub((size_t)data_len + 2, std::memory_order_relaxed);
    ...
    UnifiedPointer WriteMem(Key_t key, const std::string_view& value) {
        if (value.size() > 0 && value.size() <= 7) {
            ...
            uint64_t final_val = (static_cast<uint64_t>(value.size()) << 54) | (inline_val & ((1ULL << 54) - 1));
            return UnifiedPointer::FromPMem(final_val);
        }

        if (value.size() > std::numeric_limits<uint16_t>::max()) return UnifiedPointer();
        ...
        uint8_t lenle[2] = { (uint8_t)(data_len & 0xFF), (uint8_t)((data_len >> 8) & 0xFF) };
        std::memcpy(ptr, lenle, 2);
        if (data_len) std::memcpy(ptr + 2, value.data(), data_len);
    ...
    UnifiedPointer WriteDisk(const std::string_view& value) {
        ...
        if (!disk_ptr) { LOG(ERROR) << "NEW SSD WRONG"; return UnifiedPointer(); }
        off_t pool_off = ssd_manage.GetMallocOffset(disk_ptr);
        if (pool_off == -1) { ssd_manage.Free(disk_ptr); return UnifiedPointer(); }
        ...
        while (!stop_.load(std::memory_order_acquire)) {
            {
                ...
                evict_cv_.wait_for(lk, std::chrono::milliseconds(20), [&]{
                    return stop_.load() || need_evict_ || shm_used_bytes_.load() > HighWatermark();
                });
            }
            if (stop_.load()) break;
            ...
            if (shm_used_bytes_.load(std::memory_order_relaxed) > LowWatermark()) {
                ...
                for (const auto& victim_key : victim_keys) {
                    if (shm_used_bytes_.load(std::memory_order_relaxed) <= LowWatermark()) break;
                    ...
                    if (swapped) FreePointer(mem_ptr);
                    else         FreePointer(ssd_ptr);
                }
            }
            ...
            size_t promote_headroom = 0;
            {
                ...
                size_t budget_floor = LowWatermark() > shm_capacity_bytes_ / 20
            ...
            if (promote_headroom > 0) {
                ...
                for (auto it = hot_.begin(); it != hot_.end(); ++it) {
                    if (scanned++ >= kPromoteScanBudget) break; // 限额扫描
                    if (it->second < kPromoteThreshold) continue;
                    ...
                    UnifiedPointer disk_ptr;
                    {
                        ...
                        if (disk_ptr.type() != UnifiedPointer::Type::Disk) { hot_.erase(key); continue; }
                    }
                    ...
                    if (swapped) {
                        ...
                        hot_.erase(key);
                        if (shm_used_bytes_.load(std::memory_order_relaxed) >= LowWatermark()) break; // 用完预算
                    } else {
                        FreePointer(mem_ptr);
                    }
                }
                ...
                // 轻度衰减少量热点分（避免全表扫描）
                size_t dec = 0;
                for (auto it = hot_.begin(); it != hot_.end() && dec < kDecayPerRound; ++it, ++dec) {
                    if (it->second > 0) it->second--;
                }
            }

            { std::lock_guard<std::mutex> lk(evict_mu_); need_evict_ = false; evict_cv_.notify_all(); }
        }
    }
};