...
void SGD::Init(const std::vector<std::string> table_name, const EmbeddingTableConfig& config) {
    for (const auto& name : table_name) {
    }
}

void SGD::Update(std::string table, const std::vector<uint64_t>& keys, const std::vector<std::vector<float>>& grads, unsigned tid) {
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        std::string value((char*)updated_values[i].data(), updated_values[i].size() * sizeof(float));
        it->second->Put(keys[i], value, tid);
    }
}

void AdaGrad::Init(const std::vector<std::string> table_name, const EmbeddingTableConfig& config) {
    for (const auto& name : table_name) {
    }
}

void AdaGrad::Update(std::string table, const std::vector<uint64_t>& keys, const std::vector<std::vector<float>>& grads, unsigned tid) {
    std::vector<std::vector<float>> current_values(keys.size());
    ...
    if (acc_it == tensor_map_.end()) {
        throw std::runtime_error("Accumulated gradient table not found: " + acc_table);
    }
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        if (value.empty()) {
            accumulated_gradients_squared[i] = std::vector<float>(grads[i].size(), 0.0f);
        } else {
            size_t float_count = value.size() / sizeof(float);
            if (float_count != grads[i].size()) {
                accumulated_gradients_squared[i] = std::vector<float>(grads[i].size(), 0.0f);
                ...
                float* data = (float*)value.data();
                accumulated_gradients_squared[i] = std::vector<float>(data, data + grads[i].size());
            }
        }
    }
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        for (size_t j = 0; j < grads[i].size(); ++j) {
            updated_acc_grad[j] = accumulated_gradients_squared[i][j] + grads[i][j] * grads[i][j];
            float adaptive_lr = learning_rate_ / (std::sqrt(updated_acc_grad[j]) + epsilon_);
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        std::string param_value((char*)updated_values[i].data(), updated_values[i].size() * sizeof(float));
        std::string acc_value((char*)updated_accumulated_gradients[i].data(), updated_accumulated_gradients[i].size() * sizeof(float));
...
void RowWiseAdaGrad::Init(const std::vector<std::string> table_name, const EmbeddingTableConfig& config) {
    for (const auto& name : table_name) {
    }
}

void RowWiseAdaGrad::Update(std::string table, const std::vector<uint64_t>& keys, const std::vector<std::vector<float>>& grads, unsigned tid) {
    std::vector<std::vector<float>> current_values(keys.size());
    std::vector<std::vector<float>> accumulated_gradients_squared(keys.size());
    ...
    auto acc_it = tensor_map_.find(acc_table);
    if (acc_it == tensor_map_.end()) {
        throw std::runtime_error("Row-wise accumulated gradient table not found: " + acc_table);
    }
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        updated_acc_grad[0] = accumulated_gradients_squared[i][0] + grad_square_mean;
        
        float adaptive_lr = learning_rate_ / (std::sqrt(updated_acc_grad[0]) + epsilon_);
        for (size_t j = 0; j < grads[i].size(); ++j) {
            updated[j] = current_values[i][j] - adaptive_lr * grads[i][j];
        }
