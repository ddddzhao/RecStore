...
protected:
  ...
  void SetUp() override {
    ...
    tmp_dir_ = fs::path(::testing::TempDir()) / ("ehssd_" + std::to_string(stamp));
    ...
    ASSERT_NE(index_, nullptr) << "Failed to construct ExtendibleHashSSD with path=" << tmp_dir_;
  }
...
TEST_F(SSDExtendibleHashTest, MixedConcurrencyPutGet) {
  ...
  for (int r = 0; r < READERS; ++r) {
    ths.emplace_back([&, r] {
      while (!start.load(std::memory_order_acquire)) {}
      std::uniform_int_distribution<uint64_t> pick(1'000'000ull, 1'000'000ull + WRITERS * 100'000ull - 1);
      for (int i = 0; i < OPS_PER_WRITER; ++i) {
        uint64_t k = pick(rng);
        uint64_t got = 0;
...
TEST_F(SSDExtendibleHashTest, BulkLoadLarge) {
  ...
  for (int i = 0; i < N; ++i) { keys[i] = 700000 + i; vals[i] = i * 7 + 1; }
