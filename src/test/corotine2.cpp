...
struct [[nodiscard]] generator {
  ...
  struct promise_type {
    promise_type() {}
    ~promise_type() { reinterpret_cast<T*>(&ret_val_buf_)->~T(); }
    ...
    T get_return_value() { return *reinterpret_cast<T*>(&ret_val_buf_); }
    // void *operator new(size_t sz) { return coroutine_allocator.alloc(sz); }
    // void operator delete(void *p, size_t sz) { coroutine_allocator.free(p,
    // sz); }
...
inline bool run_display_coro(int para) {
  ...
  puts("----------perf----------");
  std::cout << res * 1.0 / (test_count * Maxcorotines) << "ns per [2-switch]"
            << std::endl;
  return true;
}
