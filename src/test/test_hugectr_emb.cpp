...
g++ -std=c++17 -O2 \
    ...
    $(python3 -c 'import torch.utils.cpp_extension as e; print(" ".join(["-I" +
p for p in e.include_paths()]))') \
    -L./third_party/HugeCTR/_build/lib \
    -L/usr/local/cuda/lib64 \
    $(python3 -c 'import torch.utils.cpp_extension as e; print(" ".join(["-L" +
p for p in e.library_paths()]))') \ -ltorch -ltorch_cpu -ltorch_python -lc10
-lcublas -lcudart
*/

/* Run:
LD_LIBRARY_PATH=$(python3 -c 'import torch.utils.cpp_extension as e;
print(":".join(e.library_paths()))'):./third_party/HugeCTR/_build/lib
./build/hugectr_test
*/
...
template <typename T>
void print_vector(
    const std::string& name, const std::vector<T>& vec, size_t limit = 5) {
...
int main() {
  ...
  try {
    ...
    for (int i = 0; i < batch_size * emb_dim; ++i) {
      h_grads[i] = static_cast<float>(i % 100) * 0.01f;
    }
    ...
    check_cuda_error(cudaMemcpy(
        d_keys_ptr, h_keys.data(), keys_size_bytes, cudaMemcpyHostToDevice));
    check_cuda_error(cudaMemcpy(
        d_grads_ptr, h_grads.data(), grads_size_bytes, cudaMemcpyHostToDevice));

    HugeCTR::Tensor2<long long> d_keys(
        {(size_t)batch_size}, std::make_shared<RawPtrBuffer>(d_keys_ptr));
    HugeCTR::Tensor2<float> d_values(
        {(size_t)batch_size, (size_t)emb_dim},
