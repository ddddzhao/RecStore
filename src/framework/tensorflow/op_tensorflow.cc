...
public:
  explicit RecstoreEmbReadOp(tf::OpKernelConstruction* context)
      : OpKernel(context) {}

  void Compute(tf::OpKernelContext* context) override {
    const tf::Tensor& keys_tensor = context->input(0);
    OP_REQUIRES(context,
                tf::TensorShapeUtils::IsVector(keys_tensor.shape()),
    ...
    OP_REQUIRES_OK(
        context, context->allocate_output(0, values_shape, &values_tensor));
    ...
    try {
      ...
      context->SetStatus(
          tf::errors::Internal("Recstore EmbRead failed: ", e.what()));
    }
  }
};
...
public:
  explicit RecstoreEmbUpdateOp(tf::OpKernelConstruction* context)
      : OpKernel(context) {}
  ...
  void Compute(tf::OpKernelContext* context) override {
    ...
    OP_REQUIRES(context,
                ...
                tf::errors::InvalidArgument("Keys must be a 1-D vector."));
    OP_REQUIRES(context,
                tf::TensorShapeUtils::IsMatrix(grads_tensor.shape()),
                tf::errors::InvalidArgument("Grads must be a 2-D matrix."));
    OP_REQUIRES(context,
                keys_tensor.dim_size(0) == grads_tensor.dim_size(0),
                tf::errors::InvalidArgument(
                    "Keys and Grads must have the same size in dimension 0."));
    OP_REQUIRES(
        context,
        grads_tensor.dim_size(1) == base::EMBEDDING_DIMENSION_D,
    ...
    try {
      ...
      context->SetStatus(
          tf::errors::Internal("Recstore EmbUpdate failed: ", e.what()));
    }
  }
};

REGISTER_KERNEL_BUILDER(Name("RecstoreEmbRead").Device(tf::DEVICE_CPU),
                        RecstoreEmbReadOp);
REGISTER_KERNEL_BUILDER(Name("RecstoreEmbUpdate").Device(tf::DEVICE_CPU),
                        RecstoreEmbUpdateOp);
