...
public:
  ZipfianSampleReader(
      int tid, int key_space_M, double zipf_theta, int nr_keys_per_req)
      : nr_keys_per_req_(nr_keys_per_req) {
    LOG(INFO) << "before mehcached_zipf_init";
    mehcached_zipf_init(
        &state,
        key_space_M * 1024 * 1024LL,
        zipf_theta,
...
public:
  PetDatasetSampleReader(PetDatasetReader* dataset_reader,
                         int tid,
                         int key_space_M,
                         int nr_keys_per_req)
      : tid_(tid),
  ...
  base::MutableArray<uint64_t> fillArray(uint64_t* buffer) override {
    ...
    while (filled_key_index != nr_keys_per_req_) {
      if (cursor_ < cursor_sample_.key_num_) {
        ...
        int need_copy_key_num = std::min(
            remained_keys_in_this_sample, nr_keys_per_req_ - filled_key_index);
        // copy [cursor_, cursor + need_copy_key_num) to [filled_key_index,
        // filled_key_index+need_copy_key_num]
        std::copy_n(&cursor_sample_.keys_->at(cursor_),
                    need_copy_key_num,
...
public:
  PetDatasetInferenceReader(
      PetDatasetReader* dataset_reader, int tid, int key_space_M)
