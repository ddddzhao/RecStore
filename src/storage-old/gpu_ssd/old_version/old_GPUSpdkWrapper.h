...
__host__ static std::shared_ptr<CmdTime>
createReportingList(size_t numEntries, int device) {
...
__host__ static std::shared_ptr<CmdTime>
createReportingList(size_t numEntries) {
  CmdTime* list = nullptr;

  auto err =
      cudaHostAlloc(&list, sizeof(CmdTime) * numEntries, cudaHostAllocDefault);
  if (err != cudaSuccess) {
    throw err;
  }
...
__device__ static void
waitForIoCompletion(nvm_queue_t* cq, nvm_queue_t* sq, int* errCode) {
  const uint16_t numThreads = blockDim.x;
...
__device__ static nvm_cmd_t* prepareChunk(
    QueuePair* qp,
    nvm_cmd_t* last,
    const uint64_t ioaddr,
    uint16_t offset,
  ...
  // Calculate offsets
  const uint16_t blocksPerChunk =
      NVM_PAGE_TO_BLOCK(pageSize, blockSize, chunkPages);
  const uint64_t currBlock = NVM_PAGE_TO_BLOCK(
      pageSize, blockSize, (currChunk + threadNum) * chunkPages);

  // Prepare PRP list building
  void* prpListPtr = NVM_PTR_OFFSET(qp->prpList, pageSize, threadOffset);
  uint64_t prpListAddr =
      NVM_ADDR_OFFSET(qp->prpListIoAddr, pageSize, threadOffset);
  nvm_prp_list_t prpList =
      NVM_PRP_LIST_INIT(prpListPtr, true, pageSize, prpListAddr);

  uint64_t addrs[0x1000 /
                 sizeof(uint64_t)]; // FIXME: This assumes that page size is 4K
  for (uint32_t page = 0; page < chunkPages; ++page) {
    addrs[page] =
        NVM_ADDR_OFFSET(ioaddr, pageSize, chunkPages * threadOffset + page);
  }
...
__host__ static inline void launchMoveKernel(
    size_t pageSize,
    void* input,
    void* src,
    void* dst,
  ...
  cudaError_t err = cudaMemcpyAsync(
      src, inputPtr, chunkSize * numThreads, cudaMemcpyHostToDevice);
  if (err != cudaSuccess) {
    throw err;
  }
...
static double LoopH2DCopyAndMoveKernel(
    void* fileMap,
  ...
  const size_t sourceBufferSize =
      NVM_PAGE_ALIGN(chunkSize * numThreads, 1UL << 16);
...
__global__ static void readDoubleBuffered(
    QueuePair* qp,
    const uint64_t ioaddr,
    void* src,
    void* dst,
  ...
  while (currChunk + numThreads < numChunks) {
    ...
    last = prepareChunk(
        qp, last, ioaddr, !bufferOffset, blockOffset, currChunk + numThreads);
    ...
    // Move received chunk
    moveBytes(src,
              bufferOffset * numThreads * chunkSize,
              dst,
              currChunk * chunkSize,
...
__global__ static void readSingleBuffered(
    QueuePair* qp,
    const uint64_t ioaddr,
    void* src,
    void* dst,
...
static void printStatistics(const Settings& settings,
  ...
  const double rate =
      ((double)prop.clockRate) / 1e3; // GPU's clock frequency in MHz

  fprintf(stdout,
          "#%9s; %12s; %12s; %12s; %12s; %12s; %12s;\n",
          "size",
          "disk_lat",
          "disk_bw",
          "mem_lat",
  ...
  for (size_t i = 0; i < numChunks; ++i) {
    ...
    fprintf(stdout,
            "%10zu; %12.3f; %12.3f; %12.3f; %12.3f; %12.3f; %12.3f;\n",
            t.size,
            diskTime,
            diskBw,
            moveTime,
...
static void outputFile(BufferPtr data, size_t size, const char* filename) {
  ...
  cudaError_t err =
      cudaMemcpy(buffer.get(), data.get(), size, cudaMemcpyDeviceToHost);
  if (err != cudaSuccess) {
...
static int
useBlockDevice(const Settings& settings, const cudaDeviceProp& properties) {
  ...
  try {
    ...
    double usecs =
        LoopH2DCopyAndMoveKernel(ptr, outputBuffer, pageSize, settings);

    fprintf(stderr, "Event time elapsed    : %.3f µs\n", usecs);
    fprintf(stderr,
            "Estimated bandwidth   : %.3f MiB/s\n",
...
public:
  BaMWrapper() : ctrl(controllerPath.c_str(), 1) {
    ...
    const size_t totalBlocks =
        NVM_PAGE_TO_BLOCK(pageSize, blockSize, totalPages);

    CHECK_LE(chunkSize, ctrl.info.max_data_size)
        << "Chunk size can not be larger than controller data size";
    CHECK_LE(totalBlocks, ctrl.ns.size)
        << "Requesting read size larger than disk size";
    ...
    cudaError_t err = cudaHostRegister(
        (void*)ctrl.ctrl->mm_ptr,
        NVM_CTRL_MEM_MINSIZE,
        cudaHostRegisterIoMemory);

    if (err != cudaSuccess) {
      LOG(FATAL)
          << "Unexpected error while mapping IO memory (cudaHostRegister): "
          << cudaGetErrorString(err);
    }
  ...
  void CheckError() const {
    int h_errorCode = 0;
    XMH_CUDA_CHECK(cudaMemcpy(
        &h_errorCode, d_errorCode, sizeof(int), cudaMemcpyDeviceToHost));
    XMH_CUDA_CHECK(cudaFree(d_errorCode));

    CHECK_EQ(h_errorCode, 0)
        << "WARNING: There were NVM errors: " << nvm_strerror(h_errorCode);
  }
  ...
  double Query(void* d_outputBuffer, uint64_t startBlock, int block_num) {
    ...
    const size_t sourceBufferSize = NVM_PAGE_ALIGN(
        (settings.doubleBuffered + 1) * chunkSize * settings.numThreads,
        1UL << 16);
    // NOTE(xieminhui): adapter & segmentID can be ignored
    auto source = createDma(
        ctrl.ctrl,
        sourceBufferSize,
        settings.cudaDevice,
        settings.adapter,
