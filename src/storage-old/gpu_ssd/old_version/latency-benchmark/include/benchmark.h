...
struct Event {
   ...
   */
  double estimateIops() const { return 1e6 / averageLatencyPerCommand(); }
   ...
   */
  double averageLatencyPerBlock() const { return time.count() / blocks; }

  /*
   * Calculate average number of microseconds per command.
   */
  double averageLatencyPerCommand() const { return (time.count() / commands); }

  size_t transferSize(size_t blockSize) const { return blocks * blockSize; }
