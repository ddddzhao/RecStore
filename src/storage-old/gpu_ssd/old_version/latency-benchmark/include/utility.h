...
static size_t
calculateTransferBlocks(const Ctrl& ctrl, const Settings& settings) {
  size_t size = 0;
  ...
  case AccessUnit::PAGES:
    size = NVM_PAGE_ALIGN(settings.count * ctrl.pageSize, ctrl.blockSize) /
           ctrl.blockSize;
    break;
...
static size_t
calculateOffsetBlocks(const Ctrl& ctrl, const Settings& settings) {
  size_t offset = 0;
