...
DmaPtr allocateBuffer(const nvm_ctrl_t* ctrl, size_t size, uint32_t, uint32_t) {
  ...
  if (!nvm_ok(err)) {
    throw runtime_error(
        string("Failed to create local segment mapped for controller: ") +
        nvm_strerror(err));
  }
...
DmaPtr allocateBuffer(
    const Ctrl& ctrl, size_t size, uint32_t adapter, uint32_t segmentId) {
  if (ctrl.fdid == 0) {
...
DmaPtr deviceBuffer(const nvm_ctrl_t* ctrl, size_t size) {
  ...
  auto err = nvm_dis_dma_create(
      &dma, ctrl, size, SCI_MEMACCESS_HOST_WRITE | SCI_MEMACCESS_DEVICE_READ);
  if (!nvm_ok(err)) {
    throw runtime_error(
        string("Failed to connect to remote segment: ") + nvm_strerror(err));
  }
