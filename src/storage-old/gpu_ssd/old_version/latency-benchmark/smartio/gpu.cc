...
static void
borrowDevice(Device* device, uint64_t fdid, uint32_t adapter, int cudaDevice) {
  sci_error_t err;
...
DmaPtr Gpu::allocateBuffer(const Ctrl& ctrl, size_t size, uint32_t) const {
  ...
  if (!nvm_ok(status)) {
    throw error("Failed to map device memory for controller: " +
                string(nvm_strerror(status)));
  }
...
static size_t calculateOffset(const RemoteMemory* segment) {
  ...
  if (err != SCI_ERR_OK) {
    throw error(
        "Failed to query local segment: " + string(SCIGetErrorString(err)));
  }
  ...
  if (err != SCI_ERR_OK) {
    throw error(
        "Failed to query remote segment: " + string(SCIGetErrorString(err)));
  }
...
DmaPtr Gpu::allocateBufferAndMap(
    const Ctrl& ctrl, size_t size, uint32_t segmentId) const {
  if (ctrl.fdid == 0) {
  ...
  if (err != SCI_ERR_OK) {
    errStr =
        "Failed to create local segment: " + string(SCIGetErrorString(err));
    goto returnDeviceHandle;
  }

  SCIAttachPhysicalMemory(
      0, devicePointer, 0, size, segment->local, SCI_FLAG_CUDA_BUFFER, &err);
  if (err != SCI_ERR_OK) {
    errStr = "Failed to attach device memory to segment: " +
             string(SCIGetErrorString(err));
    goto removeSegment;
  }

  SCIPrepareSegment(segment->local, adapter, 0, &err);
  if (err != SCI_ERR_OK) {
    errStr = "Failed to prepare segment on adapter: " +
             string(SCIGetErrorString(err));
    goto removeSegment;
  }
  ...
  SCIConnectDeviceSegment(
      ...
      &err);
  if (err != SCI_ERR_OK) {
    errStr = "Failed to connect to remote device memory: " +
             string(SCIGetErrorString(err));
    goto removeSegment;
  }

  segment->size = SCIGetRemoteSegmentSize(segment->bar);
  mappedPointer = SCIMapRemoteSegment(
      segment->bar,
      &segment->mapping,
      0,
      segment->size,
      nullptr,
      /*SCI_FLAG_IO_MAP_IOSPACE*/ 0,
      &err);
  if (err != SCI_ERR_OK) {
    errStr =
        "Failed to map remote device memory: " + string(SCIGetErrorString(err));
  ...
  if (err != SCI_ERR_OK) {
    errStr = "Failed to set segment available on adapter: " +
             string(SCIGetErrorString(err));
    goto unmapSegment;
  }

  status =
      nvm_dis_dma_map_local(&dma, ctrl.handle, adapter, segment->local, false);
  if (!nvm_ok(status)) {
    errStr = "Failed to map device memory segment for controller: " +
             string(nvm_strerror(status));
    goto unavailable;
  }
