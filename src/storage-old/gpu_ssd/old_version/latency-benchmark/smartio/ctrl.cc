...
CtrlManager::CtrlManager(
    uint64_t fdid, uint32_t adapter, uint32_t, bool admin, uint32_t ns)
  ...
  try {
    ...
    if (!nvm_ok(status)) {
      throw error("Failed to create admin queue memory: " +
                  string(nvm_strerror(status)));
    }
    ...
    if (!nvm_ok(status)) {
      throw error(
          "Failed to initialize admin queues: " + string(nvm_strerror(status)));
    }

    // Create controller reference wrapper
    auto* ptr = new (std::nothrow) Ctrl(
        fdid,
        nvm_dis_node_from_ctrl(ctrl),
        adapter,
        ctrl,
        nullptr,
        dma,
