...
static inline size_t consumeCompletions(const QueuePair* queue) {
  ...
  while ((cpl = nvm_cq_dequeue(cq)) != nullptr) {
    ...
    if (!NVM_ERR_OK(cpl)) {
      throw error("Queue " + to_string(queue->no) +
                  " got error: " + string(nvm_strerror(NVM_ERR_STATUS(cpl))));
    }
...
static void flush(const QueuePair* queue, uint32_t ns) {
  ...
  *cmd = local;
  ...
  if (!NVM_ERR_OK(cpl)) {
    throw error(
        "Queue " + to_string(queue->no) + " got error while flushing: " +
        string(nvm_strerror(NVM_ERR_STATUS(cpl))));
  }
...
static Event
sendWindow(const TransferPtr& transfer,
           ChunkPtr& from,
           const ChunkPtr& to,
           uint32_t ns,
           bool write) {
  ...
  const size_t prp_offset =
      NVM_SQ_PAGES(queue->getController().handle, queue->depth);
  ...
  for (numCmds = 0; numCmds < queue->depth - 1 && from != to; ++numCmds,
      ++from) {
    nvm_cmd_t* cmd = nvm_sq_enqueue(&queue->sq);
    if (cmd == nullptr) {
      throw error("Queue is full, this should not happen!");
    ...
    nvm_cmd_header(&local,
                   NVM_DEFAULT_CID(&queue->sq) * queue->no,
                   write ? NVM_IO_WRITE : NVM_IO_READ,
                   ns);
    nvm_cmd_rw_blks(&local, chunk.startBlock, chunk.numBlocks);

    nvm_cmd_data(
        &local, 1, &prpList, chunk.numPages, &buffer->ioaddrs[chunk.startPage]);
...
static void measureBandwidth(
  ...
  for (size_t i = 0; i < settings.outerIterations; ++i) {
    ...
    for (const auto& chunk : transfer->chunks) {
      ...
      if (numCmds == queue->depth - 1 ||
          (cmd = nvm_sq_enqueue(sq)) == nullptr) {
        nvm_sq_submit(sq);
        // std::this_thread::yield();

        while (numCmds == queue->depth - 1 ||
               (cmd = nvm_sq_enqueue(sq)) == nullptr) {
      ...
      nvm_cmd_data(&local,
                   ...
                   &prpList,
                   chunk.numPages,
                   &buffer->ioaddrs[chunk.startPage]);

      // Write local command to remote memory pointer in one go (write
      // combining)
      *cmd = local;
...
void benchmark(EventMap& times,
  ...
  fprintf(stderr,
          "Running %s benchmark (%s, %s, %zu iterations)... ",
