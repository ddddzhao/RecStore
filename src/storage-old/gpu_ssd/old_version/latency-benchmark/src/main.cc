...
static QueuePtr
createQueue(const CtrlPtr& ctrl, const QueueParam& qp, Settings& settings) {
  ...
  case QueueLocation::LOCAL:
    return make_shared<LocalQueue>(
        ctrl, qp.no, depth, chunk, ctrl->adapter, settings.id());

  case QueueLocation::REMOTE:
    return make_shared<RemoteQueue>(
        ctrl, qp.no, depth, chunk, ctrl->adapter, settings.id());

  case QueueLocation::GPU:
    return make_shared<GpuQueue>(
        ctrl,
        qp.no,
        depth,
        chunk,
        make_shared<Gpu>(qp.fdid, qp.adapter),
        qp.adapter,
        settings.id(),
        settings.id());
...
static void
prepareQueues(QueueMap& queues, const CtrlPtr& ctrl, Settings& settings) {
  ...
  for (const auto& qp : settings.queues) {
    if (queues.find(qp.no) != queues.end()) {
      throw runtime_error(
          "Queue number " + to_string(qp.no) + " is already used!");
    }
    ...
    if (settings.transferInfo) {
      fprintf(stderr,
              "- QUEUE no=%02u location=%s node=%u cmds=%zu prps=%zu",
              queue->no,
              queue->type().c_str(),
              queue->getNodeId(),
              queue->depth,
...
static size_t prepareChunks(
  ...
  const size_t blocksPerChunk =
      NVM_PAGE_TO_BLOCK(ctrl.pageSize, ctrl.blockSize, queue->pages);

  while (currBlock != numBlocks) {
    size_t blocks = std::min(numBlocks - currBlock, blocksPerChunk);
    size_t pages =
        NVM_PAGE_ALIGN(blocks * ctrl.blockSize, ctrl.pageSize) / ctrl.pageSize;
    size_t currPage =
        NVM_BLOCK_TO_PAGE(ctrl.pageSize, ctrl.blockSize, currBlock);
...
static MemoryPtr readFile(const string& filename, size_t size) {
  ...
  if (fp == nullptr) {
    throw runtime_error(
        "Failed to open file for read: " + string(strerror(errno)));
  }
...
static MemoryBufferPtr
createBuffer(const Ctrl& ctrl, const GpuPtr& gpu, Settings& settings) {
  ...
  if (settings.transferInfo) {
    fprintf(stderr,
            "- BUFFER location=%s size=%zu prps=%zu blocks=%zu",
            gpu == nullptr ? "ram" : "gpu",
            buffer->size,
            buffer->buffer->n_ioaddrs,
            buffer->size / ctrl.blockSize);
    if (gpu != nullptr) {
      fprintf(stderr,
              " (device=%d fdid=%lx bdf=%s name=%s)",
              gpu->cudaDevice,
              gpu->fdid,
              gpu->deviceBdf().c_str(),
              gpu->deviceName().c_str());
...
static void prepareTransfers(
    TransferMap& transfers,
    const QueueMap& queues,
    const Ctrl& ctrl,
    const MemoryPtr& fileData,
    Settings& settings) {
  ...
  if (startBlock + numBlocks > ctrl.namespaceSize) {
    throw runtime_error(
        "Specified offset and length is larger than the size of the namespace");
  }
  ...
  if (settings.transferInfo) {
    fprintf(stderr, "\n");
    fprintf(stderr,
            "- CTRL fdid=%lx node=%u namespace=%u total-blocks=%zu "
            "total-prps=%zu\n",
            ctrl.fdid,
            ctrl.nodeId,
            ctrl.namespaceId,
  ...
  if (gpu != nullptr && settings.transferInfo) {
    fprintf(stderr,
            "- GPU device=%d fdid=%lx node=%u bdf=%s name=%s\n",
            gpu->cudaDevice,
            gpu->fdid,
            gpu->nodeId,
            gpu->deviceBdf().c_str(),
  ...
  for (auto it = queues.cbegin(); it != queues.cend(); ++it) {
    ...
    if (settings.parallel) {
      ...
      if (it == queues.cend()) {
        transfer->offset =
            startBlock +
            NVM_PAGE_TO_BLOCK(ctrl.pageSize, ctrl.blockSize, currentPage);
        transfer->count =
            numBlocks -
            NVM_PAGE_TO_BLOCK(ctrl.pageSize, ctrl.blockSize, currentPage);
      } else {
        transfer->offset =
            startBlock +
            NVM_PAGE_TO_BLOCK(ctrl.pageSize, ctrl.blockSize, currentPage);
        transfer->count = NVM_PAGE_TO_BLOCK(
            ctrl.pageSize, ctrl.blockSize, totalPages / queues.size());
      }
    ...
    if (transfer->count == 0) {
      throw runtime_error(
          "Not enough data for multiple queues, specify larger data count");
    }

    currentPage += prepareChunks(
        transfer->chunks,
        transfer->queue,
        transfer->pageOffset,
        transfer->offset,
    ...
    if (settings.random) {
      std::shuffle(transfer->chunks.begin(),
                   transfer->chunks.end(),
    ...
    if (settings.transferInfo) {
      fprintf(
          stderr,
          "- TRANSFER queue=%02u pattern=%s total-cmds=%zu total-blocks=%zu "
          "total-prps=%zu block-start=%zu block-count=%zu prp-start=%zu "
          "prp-count=%zu\n",
          q->no,
          settings.random ? "random" : "sequential",
          transfer->chunks.size(),
          transfer->count * settings.innerIterations,
          NVM_BLOCK_TO_PAGE(ctrl.pageSize,
                            ctrl.blockSize,
                            transfer->count * settings.innerIterations),
          transfer->offset,
          transfer->count,
          transfer->pageOffset,
          NVM_BLOCK_TO_PAGE(ctrl.pageSize, ctrl.blockSize, transfer->count));
    }
...
static void
writeBuffer(const string& filename, const MemoryPtr& buffer, size_t size) {
  FILE* fp = fopen(filename.c_str(), "wb");
  if (fp == nullptr) {
    throw runtime_error(
        "Failed to open file for write: " + string(strerror(errno)));
  }
  ...
  fclose(fp);
  if (written != size) {
    throw runtime_error("Failed to write " + to_string(size) +
                        " bytes to file `" + filename + "'");
  }
}

static void writeFile(
    const TransferMap& transfers, const Ctrl& ctrl, const Settings& settings) {
...
static void verifyTransfers(const MemoryPtr& fileContent,
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (settings.ctrl.fdid != 0) {
      fprintf(stderr,
              "Taking controller reference (%s)... ",
              settings.manager ? "local admin" : "remote admin");
      manager = make_shared<CtrlManager>(
          settings.ctrl.fdid,
          settings.ctrl.adapter,
          settings.id(),
          settings.manager,
          settings.nvmNamespace);
    } else {
      fprintf(stderr, "Resetting controller... ");
      manager =
          make_shared<CtrlManager>(settings.ctrl.path, settings.nvmNamespace);
    }
    fprintf(stderr, "DONE\n");
