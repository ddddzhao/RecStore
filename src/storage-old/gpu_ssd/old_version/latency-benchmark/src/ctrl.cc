...
Ctrl::Ctrl(uint64_t fdid,
  ...
  int status = nvm_admin_ctrl_info(
      adminRef,
      &ctrlInfo,
      NVM_DMA_OFFSET(adminQueues, 2),
      adminQueues->ioaddrs[2]);
  if (!nvm_ok(status)) {
    throw error(
        "Failed to identify controller: " + string(nvm_strerror(status)));
  }

  // Identify namespace
  nvm_ns_info nsInfo;
  status = nvm_admin_ns_info(
      adminRef,
      &nsInfo,
      ns,
      NVM_DMA_OFFSET(adminQueues, 2),
  ...
  if (!nvm_ok(status)) {
    throw error(
        "Failed to identify namespace: " + string(nvm_strerror(status)));
  }
  ...
  if (!nvm_ok(status)) {
    throw error(
        "Failed to get number of queues: " + string(nvm_strerror(status)));
  }
...
CtrlManager::CtrlManager(const string& path, uint32_t ns)
    : controller(nullptr), fileDescriptor(-1) {
  ...
  try {
    ...
    if (!nvm_ok(status)) {
      throw error("Failed to get controller reference: " +
    ...
    if (!nvm_ok(status)) {
      throw error(
          "Failed to reset controller: " + string(nvm_strerror(status)));
    }

    // Create controller reference wrapper
    auto* ptr = new (std::nothrow)
        Ctrl(0, 0, 0, ctrl, queueMemory, queueMemory.get(), ref, ns);
    if (ptr == nullptr) {
