...
DmaPtr allocateBuffer(const nvm_ctrl_t* ctrl, size_t size) {
  ...
  if (!nvm_ok(err)) {
    throw error(
        "Failed to map local RAM for controller: " + string(nvm_strerror(err)));
  }
...
MemoryBuffer::MemoryBuffer(const Ctrl& ctrl, size_t size)
    : size(size), buffer(allocateBuffer(ctrl, size)) {}

MemoryBuffer::MemoryBuffer(const Ctrl& ctrl, size_t size, uint32_t segmentId)
    : size(size), buffer(allocateBuffer(ctrl, size, segmentId)) {}
...
void MemoryBuffer::clear() { memset(buffer->vaddr, 0, size); }
