...
static void percentiles(
    Percentiles& percentiles, vector<double>& data, const vector<double>& kth) {
...
static void printSummary(
    const Ctrl& ctrl, uint16_t queueNo, const EventList& events, bool write) {
  ...
  if (queueNo != 0) {
    fprintf(stderr,
            "Queue #%02u %s percentiles (%zu samples)\n",
            queueNo,
            write ? "write" : "read",
            events.size());
  } else {
    fprintf(stderr,
            "Aggregated %s percentiles (%zu samples)\n",
            write ? "write" : "read",
            events.size());
  }
...
void calculatePercentiles(
    const Ctrl& ctrl, const EventMap& events, bool write) {
  if (events.empty()) {
    return;
  }
...
static void printGpuMetadata(FILE* fp, const Gpu& gpu) {
  fprintf(fp,
          "### gpu[%d]: device=%d; fdid=%lx; node=%u; bdf=%s; name='%s';\n",
          gpu.cudaDevice,
          gpu.cudaDevice,
          gpu.fdid,
          gpu.nodeId,
...
static void showTransferMetadata(
    FILE* fp,
  ...
  for (const auto& tp : transfers) {
    ...
    fprintf(fp,
            "### queue[%x]: no=%x; cmds=%zu; prps=%zu; transfer-size=%zu; "
            "remote=%s; memory=%s; node=%u;\n",
            queueNo,
            queue->no,
            queue->depth,
            queue->pages,
  ...
  for (const auto& tp : transfers) {
    const GpuQueue* queue =
        dynamic_cast<const GpuQueue*>(tp.second->queue.get());
    if (queue != nullptr &&
        gpus.find(queue->getGpu().cudaDevice) == gpus.end()) {
    ...
    const GpuBuffer* buffer =
        dynamic_cast<const GpuBuffer*>(tp.second->buffer.get());
  ...
  for (const auto& tp : transfers) {
    ...
    fprintf(
        fp,
        "### transfer[%x]: queue=%x; pattern=%s; buffer=%s; buffer-prps=%zu; "
        "prp-offset=%zu; block-count=%zu; block-offset=%zu; cmds=%zu;\n",
        queueNo,
        transfer->queue->no,
  ...
  fprintf(
      fp,
      "### benchmark: ctrl=%s; ctrl-node=%u; block-size=%zu; prp-size=%zu; "
      "type=%s; write=%s; verified=%s; parallel=%s; shared=%s; random=%s; "
      "repeat=%zu; iterations=%zu\n",
      ctrlString.c_str(),
      ctrl.nodeId,
      ctrl.blockSize,
      ctrl.pageSize,
...
static void printRecords(
  ...
  for (const auto& event : events) {
    fprintf(
        fp,
        "  %4s; %5s; %8zu; %12.3f; %12zu; %12zu; %12zu; %12.3f; %12.3f; "
        ...
        event.time.count(),
        event.blocks,
        event.blocks / blocksPerPrp,
        event.transferSize(blockSize),
        event.averageLatencyPerCommand(),
        event.averageLatencyPerBlock(),
        event.averageLatencyPerBlock() * blocksPerPrp,
        event.bandwidth(blockSize),
        event.estimateIops(),
        event.adjustedIops(),
        event.adjustedIops(blocksPerPrp));
  }
}

void printStatistics(const Ctrl& ctrl,
  ...
  fprintf(
      fp,
      "#%5s; %5s; %8s; %12s; %12s; %12s; %12s; %12s; %12s; %12s; %12s; %12s; "
      "%12s; %12s;\n",
      "queue",
      "rd/wr",
      "commands",
  ...
  for (const auto& tp : transfers) {
    ...
    if (settings.write) {
      printRecords(
          fp, queueNo, *writeEvents.at(queueNo), pageSize, blockSize, true);
    }

    printRecords(
        fp, queueNo, *readEvents.at(queueNo), pageSize, blockSize, false);
    fflush(fp);
  }
