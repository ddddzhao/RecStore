...
static bool tryDeviceId(const string& str,
                        uint64_t& fdid,
                        uint32_t& adapter,
                        bool adapterRequired = false) {
  vector<string> words;
  split(words, str, ':');

  if (words.empty() || words.size() > 2 ||
      (adapterRequired && words.size() != 2)) {
    return false;
  }
...
QueueParam::QueueParam(const string& arg) {
  ...
  for (const string& w : words) {
    ...
    if (param.size() != 2) {
      throw error("`" + arg + "' is a malformed queue specification string, `" +
                  w + "' is not key=value format");
    }
    ...
    } else if (param[0] == "d" || param[0] == "c" || param[0] == "cmds" ||
               param[0] == "depth" || param[0] == "commands") {
      if (!tryNumber(depth, param[1], 0)) {
        throw error("`" + param[1] +
                    "' is an invalid number of commands (queue depth)");
      }
    ...
    } else if (param[0] == "l" || param[0] == "h" || param[0] == "loc" ||
               param[0] == "location" || param[0] == "host") {
...
static void qsInfo(std::ostringstream& s,
                   const string& name,
                   const string& argument,
                   const string& info) {
  using namespace std;
  s << "  " << left << setw(16) << name << setw(16) << argument << setw(40)
    << info << endl;
}

static void argInfo(std::ostringstream& s,
                    const string& name,
                    const string& argument,
                    const string& info) {
...
static void
argInfo(std::ostringstream& s, const string& name, const string& info) {
  argInfo(s, name, "", info);
}
...
static string helpString(const string& progname) {
  ...
  s << "Usage: " << progname
    << " --ctrl <path> --queue <string>... --blocks=<count>" << endl;
#ifdef __DIS_CLUSTER__
  s << "   or: " << progname
    << " --ctrl <fdid>[:<ntb>] [--client] --queue <string>... --blocks=<count>"
    << endl;
#endif
  s << endl;
  ...
  argInfo(s,
          "ctrl",
          "<fdid>[:<ntb>]",
          "NVM controller fabric ID and NTB adapter number");
  argInfo(
      s, "rpc", "request I/O queues from admin and do not reset controller");
  argInfo(s, "client", "alias for --rpc");
  argInfo(
      s, "segment", "<id>", "specify segment identifier offset (default 0)");
#endif
  argInfo(s,
          "queue",
          "<string>",
          "specify I/O queue (can be repeated), see queue specification");
  ...
  //    argInfo(s, "commands", "[<count>]", "specify data unit as number of I/O
  //    commands"); argInfo(s, "cmds", "[<count>]", "alias for --commands");
  ...
  argInfo(
      s, "gpu", "<device>", "host memory on local CUDA device instead of RAM");
#ifdef __DIS_CLUSTER__
  argInfo(s,
          "gpu",
          "<fdid>[:<ntb>]",
          "host memory on remote CUDA device instead of RAM (NB! must be "
          "borrowed)");
#endif
  ...
  argInfo(s,
          "iterations",
          "<count>",
          "repeat benchmark <count> times (default is 1000)");
  argInfo(s, "outer", "<count>", "alias for --iterations");
  argInfo(
      s, "repeat", "<count>", "repeat commands <count> times (default is 1)");
  ...
  argInfo(s,
          "infile",
          "<filename>",
          "load file into buffer before reading from disk");
  argInfo(s, "input", "<filename>", "alias for --infile");
  argInfo(
      s, "write", "write to disk before reading (NB! destroys data on disk)");
  argInfo(s, "verify", "verify disk read/write");

  s << endl;
  s << "Queue specification string" << endl;
  s << "  The queue specification is a CSV string, where each parameter is "
       "formatted as key-value pair."
    << endl;
  s << "  For example: `no=1,cmds=32,prps=1,location=local' or `no=3,depth=1'"
    << endl;
  s << endl;
  ...
  qsInfo(
      ...
      "cmds",
      "<commands>",
      "number of unsubmitted I/O commands (defaults to controller's maximum)");
  qsInfo(s, "depth", "<commands>", "alias for cmds");
  qsInfo(s,
         "prps",
         "<pages>",
         "data transfer size in number of PRPs/pages (defaults to controller's "
         "maximum for sequential and 1 for random)");
  ...
  qsInfo(s,
         "location",
         "remote",
         "host submission queue in remote RAM close to controller");
  qsInfo(s,
         "location",
         "<fdid>[:<ntb>]",
         "host submission queue in remote CUDA device memory (NB! must be "
         "borrwed)");
#endif
  s << endl;
...
void Settings::parseArguments(int argc, char** argv) {
  ...
  try {
    ...
    while ((option = getopt_long(
                argc, argv, optstr.c_str(), options, &index)) != -1) {
      switch (option) {
      case 'h':
      ...
      case ':':
        throw error("Missing argument value for option `" +
