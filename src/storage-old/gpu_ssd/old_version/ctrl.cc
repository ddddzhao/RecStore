...
static void initializeController(struct Controller& ctrl, uint32_t ns_id) {
  ...
  // Identify controller
  status = nvm_admin_ctrl_info(
      ctrl.aq_ref,
      &ctrl.info,
      NVM_DMA_OFFSET(ctrl.aq_mem, 2),
      ctrl.aq_mem->ioaddrs[2]);
  if (!nvm_ok(status)) {
...
Controller::Controller(uint64_t ctrl_id, uint32_t ns_id, uint32_t, uint32_t)
  ...
  if (!nvm_ok(status)) {
    throw error(
        string("Failed to get controller reference: ") + nvm_strerror(status));
  }
...
void Controller::reserveQueues() { reserveQueues(n_sqs, n_cqs); }
