...
static void createLongOptions(
    vector<option>& options, string& optionString, const OptionMap& parsers) {
...
void Settings::parseArguments(int argc, char** argv) {
  OptionMap parsers = {
      ...
      {'c',
       OptionPtr(new Option<uint64_t>(
           controllerId, "fdid", "ctrl", "NVM controller device identifier"))},
      {'f',
       OptionPtr(new Option<uint64_t>(
           cudaDeviceId, "fdid", "fdid", "CUDA device FDID"))},
      {'a',
       OptionPtr(new Option<uint32_t>(
           adapter, "number", "adapter", "DIS adapter number", "0"))},
      {'S',
       OptionPtr(new Option<uint32_t>(
           segmentId,
           "offset",
           "segment",
           "DIS segment identifier offset",
           "0"))},
#else
  ...
  //controller device path"))},
      ...
      {'g',
       OptionPtr(new Option<uint32_t>(
           cudaDevice, "number", "gpu", "specify CUDA device", "0"))},
      {'k',
       OptionPtr(new Option<uint32_t>(
           n_ctrls,
           "number",
           ...
           "1"))},
      //{'i', OptionPtr(new Option<uint32_t>(nvmNamespace, "identifier",
      //"namespace", "NVM namespace identifier", "1"))},
      ...
      {'q',
       OptionPtr(new Range(
           ...
           65536,
           "num_queues",
           "number of queues per controller",
           "1"))},
      {'e',
       OptionPtr(new Range(
           numElems,
           ...
           "2147483648"))},
      {'r',
       OptionPtr(new Option<bool>(
           random,
           "bool",
           "random",
           "if true the random access benchmark runs, if false the sequential "
           "access benchmark runs",
           "true"))},
      //{'o', OptionPtr(new Option<const char*>(output, "path", "output",
      //"output read data to file"))},
      //{'s', OptionPtr(new Option<uint64_t>(startBlock, "offset", "offset",
      //"number of blocks to offset", "0"))},
      //{'j', OptionPtr(new Option<const char*>(blockDevicePath, "path",
      //"block-device", "path to block device"))}
  };
  ...
  while ((option = getopt_long(
              argc, argv, optionString.c_str(), &options[0], &index)) != -1) {
    switch (option) {
    case '?':
