...
public:
  static void Initialize(const char* pool_name, size_t pool_size) {
    ...
    if (!pm_allocator::FileExists(pool_name)) {
      ...
      pm_pool = pmemobj_create_addr(
          pool_name, layout_name, pool_size, CREATE_MODE_RW, (void*)pool_addr);
      if (pm_pool == nullptr) {
...
public:
  ...
  template <class U>
  using rebind = PMAllocator<U>;

  PMAllocator()                          = default;
  PMAllocator(const PMAllocator& aOther) = default;
  template <class O>
  PMAllocator(const PMAllocator<O>& aOther){};
  ~PMAllocator() = default;
};
