...
extern hash_api* create_hashtable_pclht(
    const hashtable_options_t& opt, unsigned sz, unsigned tnum);
extern hash_api* create_hashtable_level(
    const hashtable_options_t& opt, unsigned sz, unsigned tnum);
extern hash_api* create_hashtable_clevel(
    const hashtable_options_t& opt, unsigned sz, unsigned tnum);
extern hash_api* create_hashtable_cceh(
    const hashtable_options_t& opt, unsigned sz, unsigned tnum);
extern hash_api* create_hashtable_ccehvm(
    const hashtable_options_t& opt, unsigned sz, unsigned tnum);
...
public:
...
#endif
  {
    ...
    if (config.hash_name == "clht") {
      hash_table_ = create_hashtable_pclht(
          hashtable_options, config.hash_size, config.num_threads);
    } else if (config.hash_name == "level") {
      hash_table_ = create_hashtable_level(
          hashtable_options, config.hash_size, config.num_threads);
    } else if (config.hash_name == "clevel") {
      hash_table_ = create_hashtable_clevel(
          hashtable_options, config.hash_size, config.num_threads);
      ...
      hash_table_ = create_hashtable_ccehvm(
          hashtable_options, config.hash_size, config.num_threads);
    } else {
  ...
  void Get(const uint64_t key, std::string& value, unsigned tid) override {
    ...
    if (hash_table_->find(
            (char*)&key, sizeof(uint64_t), (char*)&read_value, tid) == false) {
      value = std::string();
    } else {
  ...
  void BatchGet(base::ConstArray<uint64> keys,
    ...
    for (auto k : keys) {
      ...
      // auto epoch_guard = Allocator::AquireEpochGuard();
      if (hash_table_->find(
              (char*)&k, sizeof(uint64_t), (char*)&read_value, tid) == false) {
        // printf("key: %d\n", (int)k);
