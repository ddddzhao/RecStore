...
class KVEngineDoubleDesk : public BaseKV {
  ...
  constexpr static int MAX_COROTINE_SIZE =
      MAX_THREAD_CNT * MAXCOROTINE_SIZE_PERTHREAD;
...
#  endif
  ...
  void SubBulkLoad(int keys_size,
                   base::ConstArray<uint64_t> indexs_array,
                   std::vector<base::ConstArray<float>>& value,
                   int start,
                   char* pinned_value,
                   int tid) {
    CHECK(keys_size == indexs_array.Size());
...
public:
  explicit KVEngineDoubleDesk(const BaseKVConfig& config)
      : BaseKV(config),
        ...
        per_thread_buffer_size(
            (long long)value_size * (long long)max_batch_keys_size),
    ...
    CHECK_LE(corotine_num, MAX_COROTINE_SIZE)
        << "corotine_num must be less than " << MAX_COROTINE_SIZE;
    ...
    rawbouncedBuffer_ = (char*)spdk_malloc(
        kBouncedBuffer_ * ssd_->GetLBASize() * corotine_num,
        0,
        NULL,
  ...
  void BatchGet(coroutine<void>::push_type& sink,
    ...
    for (int i = 0; i < keys.Size(); i++) {
      ...
      if (info->in_cache) {
        info->hit_cnt++;
        values->emplace_back((float*)(cache_ + info->cache_offset * value_size),
        ...
        ssd_->SubmitReadCommand(
            bouncedBuffer_[rt] + unhit_size * ssd_->GetLBASize(),
            value_size,
            lba_no,
            ReadCompleteCB,
            &readCompleteCount,
        ...
        values->emplace_back(
            (float*)(bouncedBuffer_[rt] + unhit_size * ssd_->GetLBASize() +
                     in_lba_offset),
            value_size / sizeof(float));
    ...
    for (int i = free_pos.first; i != free_pos.second;
      ...
      memcpy(cache_ + pos * value_size,
             bouncedBuffer_[rt] + j * ssd_->GetLBASize() + in_lba_offset,
             value_size);
      j++;
    }
    cache_timer.CumEnd();
  ...
  void BatchPut(coroutine<void>::push_type& sink,
    ...
    for (int i = 0; i < keys.Size(); i++) {
      ...
      if (info->in_cache) {
        memcpy(cache_ + info->cache_offset * value_size,
               values[i].Data(),
               value_size);
      }
    }
...
FACTORY_REGISTER(
    BaseKV, KVEngineDoubleDesk, KVEngineDoubleDesk, const BaseKVConfig&);