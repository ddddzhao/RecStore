...
static bool check_eq_2d(const std::vector<std::vector<float>>& a, const std::vector<std::vector<float>>& b) {
    if (a.size() != b.size()) return false;
    for (int i = 0; i < a.size(); i++) {
...
void TestBasicConfig() {
    ...
    // 测试 recstore 配置格式
    json recstore_config = {{"distributed_client",
                             {{"servers",
    ...
    try {
        ...
        std::cout << "Recstore bRPC config parsed successfully, shard count: " << client.shard_count() << std::endl;
...
void TestFactoryClient() {
    ...
    json config = {{"distributed_client",
                    {{"servers",
                      {{{"host", "127.0.0.1"}, {"port", 15000}, {"shard", 0}},
                       {{"host", "127.0.0.1"}, {"port", 15001}, {"shard", 1}}}},
                     {"num_shards", 2},
    ...
    std::unique_ptr<BasePSClient> base_client(
        base::Factory<BasePSClient, json>::NewInstance("distributed_brpc", config));

    if (!base_client) {
        std::cerr << "Failed to create distributed bRPC PS client via factory!" << std::endl;
        return;
    }

    std::cout << "Successfully created distributed bRPC PS client via factory" << std::endl;

    // 转换为子类指针以访问扩展接口
    auto* client = dynamic_cast<DistributedBRPCParameterClient*>(base_client.get());
    if (!client) {
        std::cerr << "Failed to cast to DistributedBRPCParameterClient!" << std::endl;
        return;
    }
...
void TestDirectClient() {
    ...
    try {
        ...
        std::cout << "Direct bRPC client created successfully, shard count: " << client.shard_count() << std::endl;
