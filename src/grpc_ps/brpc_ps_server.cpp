...
DEFINE_string(brpc_config_path, RECSTORE_PATH "/recstore_config.json", "config file path");
DEFINE_int32(brpc_server_port, 15000, "bRPC server port");
DEFINE_int32(brpc_server_num_threads, 0, "Number of threads for bRPC server, 0 means auto");

namespace recstore {

BRPCParameterServiceImpl::BRPCParameterServiceImpl(CachePS* cache_ps) : cache_ps_(cache_ps) {
...
void BRPCParameterServiceImpl::PrintMetrics(const std::string& table_name, const std::string& unique_id) {
...
void BRPCParameterServiceImpl::GetParameter(google::protobuf::RpcController* controller,
    ...
    FB_LOG_EVERY_MS(INFO, 1000) << "[bRPC PS] Getting " << keys_array.Size() << " keys";
...
void BRPCParameterServiceImpl::Command(google::protobuf::RpcController* controller,
...
void BRPCParameterServiceImpl::PutParameter(google::protobuf::RpcController* controller,
    ...
    const ParameterCompressReader* reader =
        reinterpret_cast<const ParameterCompressReader*>(request->parameter_value().data());
...
void BRPCParameterServiceImpl::UpdateParameter(google::protobuf::RpcController* controller,
...
void BRPCParameterServiceImpl::InitEmbeddingTable(google::protobuf::RpcController* controller,
...
public:
    ...
    void Run() {
        ...
        if (num_shards > 1) {
            // 多服务器启动逻辑
            std::cout << "启动分布式参数服务器 (bRPC)，分片数量: " << num_shards << std::endl;
            ...
            if (servers.size() != num_shards) {
                LOG(FATAL) << "servers 配置数量 (" << servers.size() << ") 与 num_shards (" << num_shards
                          << ") 不匹配";
                return;
            }
            ...
            for (auto& server_config : servers) {
                server_threads.emplace_back([this, server_config]() {
                    ...
                    std::cout << "bRPC Server shard " << shard << " listening on " << server_address << std::endl;
            ...
            std::string server_address = "0.0.0.0:" + std::to_string(FLAGS_brpc_server_port);
            auto cache_ps = std::make_unique<CachePS>(config_["cache_ps"]);
            auto service = std::make_unique<BRPCParameterServiceImpl>(cache_ps.get());
